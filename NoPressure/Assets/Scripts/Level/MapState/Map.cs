...
public class Map
{
    ...
    public MapTile GetMapTileAt(Coord location)
    {
        ...
        return Sector.getMapTileAt(location); 
    }
    ...
    public MapNode GetMapNodeAt(Coord ILocation, string lMapName)
    {
        ...
        if (sMapNodes[ILocation.x, ILocation.y] == null)
        {
            ...
            if(ILocation.y + 1 <= 9)
                NorthNode = sMapNodes[ILocation.x, ILocation.y + 1];

            if(ILocation.y -1 >= 0)
                SouthNode = sMapNodes[ILocation.x, ILocation.y -1];

            if (ILocation.x - 1 >= 0)
                EastNode = sMapNodes[ILocation.x -1, ILocation.y];
            
            if (ILocation.x + 1 <= 9)
                WestNode = sMapNodes[ILocation.x + 1, ILocation.y];
