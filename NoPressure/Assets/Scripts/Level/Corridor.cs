...
public class Corridor
{
    ...
    public BoardManager.Direction direction;   // Which direction the corridor is heading from it's room.
    ...
    public int EndPositionX
    {
        get
        {
            if (direction == BoardManager.Direction.North || direction == BoardManager.Direction.South)
                return startXPos;
            if (direction == BoardManager.Direction.East)
    ...
    public int EndPositionY
    {
        get
        {
            if (direction == BoardManager.Direction.East || direction == BoardManager.Direction.West)
                return startYPos;
            if (direction == BoardManager.Direction.North)
    ...
    public void SetupCorridor(Room room, string corrId, IntRange length, IntRange roomWidth, IntRange roomHeight, int columns, int rows, bool firstCorridor)
    {
        ...
        direction = (BoardManager.Direction)Random.Range(0, 4);
        ...
        BoardManager.Direction oppositeDirection = (BoardManager.Direction)(((int)room.enteringCorridor + 2) % 4);
        ...
        if (!firstCorridor && direction == oppositeDirection)
        {
            ...
            direction = (BoardManager.Direction)directionInt;

        }

        // Set a random length.
        corridorLength = length.RandomInt;
        ...
        switch (direction)
        {
            ...
            case BoardManager.Direction.North:
                // ... the starting position in the x axis can be random but within the width of the room.
                startXPos = Random.Range(room.bottom_left_x, room.bottom_left_x + room.width - 1);

                // The starting position in the y axis must be the top of the room.
                startYPos = room.bottom_left_y + room.height;
                ...
                break;
            case BoardManager.Direction.East:
                startXPos = room.bottom_left_x + room.width;
                startYPos = Random.Range(room.bottom_left_y, room.bottom_left_y + room.height - 1);
                maxLength = columns - startXPos - roomWidth.m_Min;
                break;
            case BoardManager.Direction.South:
                startXPos = Random.Range(room.bottom_left_x, room.bottom_left_x + room.width);
                startYPos = room.bottom_left_y;
                maxLength = startYPos - roomHeight.m_Min;
                break;
            case BoardManager.Direction.West:
                startXPos = room.bottom_left_x;
                startYPos = Random.Range(room.bottom_left_y, room.bottom_left_y + room.height);
